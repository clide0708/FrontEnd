...
export default function ModalAdd({
  ...
  const carregarItensAdicionados = async (idRefeicao) => {
    try {
      const response = await alimentosService.listarAlimentosRefeicao(
        idRefeicao
      );
      if (response.success) {
      ...
      try {
        ...
        if (response.success && response.refeicoes) {
          ...
          const refeicaoExistente = response.refeicoes.find((ref) => {
            ...
            console.log(
              `🔍 Comparando: ${dataRef} === ${hoje} para ${ref.nome_tipo}`
            );
          ...
          if (refeicaoExistente) {
            ...
            if (criarResponse.id_refeicao) {
              ...
              console.log(
                "✅ Nova refeição criada com ID:",
                criarResponse.id_refeicao
              );
            } else {
  ...
  const handleCriarNovaRefeicao = async (nomeRefeicao) => {
    try {
      ...
      if (response.success && response.id_refeicao) {
        console.log(
          "✅ Nova refeição criada:",
          nomeRefeicao,
          "ID:",
          response.id_refeicao
        );
        ...
        throw new Error(
          response.error || "Erro desconhecido ao criar refeição"
        );
      }
    } catch (error) {
      ...
      if (
        ...
        setErro(
          `A refeição "${nomeRefeicao}" já existe para hoje. Use "Adicionar a Refeição Existente".`
        );
      } else {
        setErro(`Erro: ${error.message}`);
      }
  ...
  const handleAdicionarARefeicaoExistente = async (nomeRefeicao) => {
    try {
      ...
      if (response.success && response.refeicoes) {
        const refeicaoExistente = response.refeicoes.find(
          (ref) => ref.nome_tipo === nomeRefeicao
        );
  ...
  const [modo, setModo] = useState(
    currentMealList === ""
      ? "selecao"
      : currentMealList === "nova"
      ? "criacao"
      : currentMealList === "existente"
      ? "adicao"
      : "especifico"
  );
  ...
  const nomeExibicao = modoCriacao
    ? "Adicionar Refeição"
    : mapearNomeRefeicao(currentMealList);
  ...
  return (
    <div className="modalAdd show">
      <div className="modalalm-content">
        <div className="addalm">
          <div className="addalmbtn">
            <h4 className="h4modal">
              {modo === "selecao" && "Adicionar Refeição"}
        ...
        {modo === "selecao" && (
          <div
            className="gerenciar-refeicoes-section"
            style={{ marginBottom: "20px" }}
          >
            <div
              className="opcoes-refeicao"
              style={{ display: "flex", flexDirection: "column", gap: "15px" }}
            >
              {/* Opção 1: Criar nova refeição */}
              <div className="opcao-criar">
                <div
                  className="botoes-refeicao"
        ...
        {modo === "criacao" && (
          <div
            className="modo-criacao-section"
            style={{ marginBottom: "20px" }}
          >
